...
\item Let \(A\) be a Noetherian domain with field of fractions \(K\). Let \(I\neq(0)\) be an ideal. Let \(I^{-1}\coloneqq\{\alpha\in K\mid \alpha i\in A,\ \forall i\in I\}\). Show that \(I^{-1}\) is a finitely generated torsion free \(A\)-module of rank 1.
    \begin{proof}
        First we will show that \(I^{-1}\coloneqq\{\alpha\in K\mid \alpha i\in A,\ forall i\in I\}\) is indeed an \(A\)-module. \begin{itemize}
            \item Let \(\alpha,\beta\in I^{-1}\), then \[
                    (\alpha+\beta)i=\alpha i+\beta i\in A\implies \alpha+\beta\in I^{-1}
                \]
            \item Let \(\alpha\in I^{-1}\), and \(a\in A\) then \[
                    (\alpha a)i=a(\alpha i)\in A
                \]
                So \(\alpha a\in I^{-1}\)
        \end{itemize}
        So \(I^{-1}\) is an \(A\)-module. Then it defines a natural exact sequence of \(A\)-modules:
        \[
            \begin{tikzcd}
                0\arrow[r] & I^{-1} \arrow[r] & A \arrow[r] & A/(I^{-1})\arrow[r] & 0
            \end{tikzcd}
        \]
        Notice that if \(I\neq(0)\), then \(I^{-1}\neq(0)\) and \(A/I^{-1}\) is torsion. Indeed, choose any \(\alpha\in I^{-1}\), then \(\alpha\neq0\in\operatorname{Ann}_{A}(A/I^{-1})\). So \(A/I^{-1}\) has a nonzero annihilator, and thus it is torsion. Now, tensoring the exact sequence with \(K\), gives another exact sequence
        \[
            \begin{tikzcd}
                0\arrow[r] & K\otimes_{A} I^{-1} \arrow[r] & K \arrow[r] & K\otimes_{A} A/I^{-1}\arrow[r] & 0
            \end{tikzcd}
        \]
        So we have \(\dim_{K}(K)=\dim_{K}(K\otimes_{A}I^{-1})+\dim_{K}(K\otimes_{A}A/I^{-1})\). Clearly \(\dim_{K}(K)=1\), and by problem 5, \(\rk_{A}(A/I^{-1})=0\) as \(A/I^{-1}\) is torsion. Therefore \(K\otimes I^{-1}\cong K\) so \(I^{-1}\) is a torsion free \(A\)-module of rank 1. Notice if \(I=(i)\) is the generator of \(I\) in \(A\), then \(I^{-1}=(\frac{1}{i})\) is the generator of \(I^{-1}\) in \(K\), so \(I^{-1}\) is indeed finitely generated.
    \end{proof}
\item
    \begin{enumerate}[label=(\alph*).]
        \item Show that if \(a,b\in\Z\), and \(\gcd(a,b)=1\), then \(\Z[\frac{a}{b}]=\Z[\frac{1}{b}]\).
        \item Show that any subring of \(\Q\) that contains \(1\) is Noetherian.
    \end{enumerate}
    \begin{enumerate}[label=(\alph*).]
        \item \begin{proof}
                Let \(a,b\in\Z\) with \(\gcd(a,b)=1\). \begin{itemize}
...
\item Let \(A\) be a commutative ring. Let \(I\) be any subset of \(A\). Let \(V(I)\coloneqq\{P\in\sp(A)\mid I\subseteq P\}\). Now let \(I_{1}\) and \(I_{2}\) be two ideals of \(A\).
    ...
    \begin{enumerate}[label=(\alph*).]
        \item Let \(P\in V(I_{1}I_{2})\). Then \(I_{1}I_{2}\subseteq P\), but \(P\in\sp(A)\) so either \(I_{1}\subseteq P\) or \(I_{2}\subseteq P\). So
            ...
            \[
                V(I_{1}\cap I_{2})\subseteq V(I_{1}I_{2})
            \]
            But if \(P\) is a prime ideal containing \(I_{1}\), it also contains all subsets of \(I_{1}\), so it must contain \(I_{1}\cap I_{2}\subseteq I_{1}\). Similarly if \(P\) contains \(I_{2}\), it must also contain the intersection. So we have \[
           V(I_{1}I_{2})\subseteq V(I_{1}\cap I_{2})
            \]
           So \[
           V(I_{1}I_{2})=V(I_{1}\cap I_{2})
           \]
           Combining, we get 
           \[
               V(I_{1}\cap I_{2})=V(I_{1}I_{2})=V(I_{1})\cup V(I_{2})
           \]
       \item Let \(P\in V(I_{1}+I_{2})\). So \(I_{1}+I_{2}=\{a+b\mid a\in I_{1},b\in I_{2}\}\subseteq P\). So clearly \(I_{1}\subseteq P\) and \(I_{2}\subseteq P\). Then \[
               V(I_{1}+I_{2})\subseteq V(I_{1})\cap V(I_{2})
       \]
      Let \(P\in V(I_{1})\cap V(I_{2})\), then both \(I_{1}\) and \(I_{2}\) are subsets of \(P\), and so \(I_{1}+I_{2}\subseteq P\). So
      \[
               V(I_{1})\cap V(I_{2})\subseteq V(I_{1}+I_{2})
      \]
     Combining, \[
      V(I_{1}+I_{2})=V(I_{1})\cap V(I_{2})
     \]
 \item Let \(V(I)\) for \(I\) an ideal be a closed set. Then we have shown:
     \begin{enumerate}
         \item Finite union,
             \[
                 V(I_{1})\cup V(I_{2})=V(I_{1}I_{2})
             \]
             is a closed set.
         \item Finite intersection
             \[
                 V(I_{1})\cap V(I_{2})=V(I_{1}+I_{2})
             \]
             is a closed set.
         \item Finally, consider the ideals \((0)\) and \(A\), clearly,
             \[
                 V((0))=\sp(A)
             \]
             as \((0)\subseteq P\) for all prime ideals \(P\). Similarly, \[
             V(A)=\emptyset
             \]
            Because no prime ideal contains all of \(A\). So the empty set and the whole space are closed sets.
     \end{enumerate}
     So the sets \(V(I)\) form a topology on \(\sp(A)\).
\end{enumerate}
\end{proof}
\item Let \(\psi:A\to B\) be a ring homomorphism. Endow \(\sp(B)\) and \(\sp(A)\) with the Zariski topology. Show that \(\sp(\psi)\) is a continuous map. Spec is a contravariant functor from the category of commutative rings to the category of topological spaces.
    \begin{proof}
